    ...
    <div class="relative w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <form class="p-4 md:p-5 text-left" wire:submit="save" novalidate>
                <!-- Almacen Select -->
                <div class="">
                    <label for="transporte_cliente_id"
                        class="block mb-1 text-sm font-medium text-gray-900 dark:text-white capitalize">Cliente o tranporte</label>
                    <select id="transporte_cliente_id" wire:model="transporte_cliente_id" value="{{ old('transporte_cliente_id') }}"
                        class="block w-full p-2 mb-6 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        <option selected>Escoge un tipo</option>
                        @foreach ($transporteClienteLista as $transporteCliente)
                            <option value="{{ $transporteCliente->id }}">{{ $transporteCliente->name }} {{ $transporteCliente->lastname }}</option>
                        @endforeach
                    </select>
                    @error('transporte_cliente_id')
                ...
                <div class="">
                    <label for="identificador"
                        class="block mb-1 text-sm font-medium text-gray-900 dark:text-white capitalize">#
                        Comprobante</label>
                ...
                <div class="">
                    <label for="observaciones"
                        class="block mb-1 text-sm font-medium text-gray-900 dark:text-white capitalize">Observaciones</label>
                        ...
                        <div class="flex items-center mt-2 space-x-2">
                            <select wire:model="detalles.{{ $index }}.item_id"
                                ...
                                @foreach ($items as $item)
                                    <option value="{{ $item->id }}">{{ $item->codigo }} {{ $item->tipo }}
                                        {{ $item->tamaño }} {{ $item->color }}</option>
                                @endforeach
                            </select>
